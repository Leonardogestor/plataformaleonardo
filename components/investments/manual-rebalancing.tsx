...
export function ManualRebalancing({ investments, className }: ManualRebalancingProps) {
  ...
  const calculateImpact = () => {
    ...
    const needsRebalancing = allocations.some(
      (item) => Math.abs(item.difference) > totalValue * 0.01
    ) // 1%

    return {
      totalDifference,
      needsRebalancing,
      affectedAssets: allocations.filter((item) => Math.abs(item.difference) > totalValue * 0.01)
        .length,
      maxMovement: Math.max(...allocations.map((item) => Math.abs(item.difference))),
    }
  }

  // Executar rebalanceamento (simulação)
      ...
      <CardContent className="space-y-6">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 p-4 bg-muted/30 rounded-lg">
          <div className="text-center">
            <div className="text-lg font-bold text-blue-600">{impact.affectedAssets}</div>
          ...
          <div className="text-center">
            <Badge
              className={
                impact.needsRebalancing ? "bg-red-100 text-red-800" : "bg-green-100 text-green-800"
              }
            >
            ...
            <div key={allocation.id} className="space-y-3 p-4 border rounded-lg">
              ...
              <div className="space-y-2">
                <Slider
                  ...
                  onValueChange={(value) => updateAllocation(allocation.id, value[0] || 0)}
                  max={100}
                  min={0}
                  step={0.1}
                  className="w-full"
              ...
              {Math.abs(allocation.difference) > totalValue * 0.01 && (
                ...
                >
                  <span>{allocation.difference > 0 ? "Aumentar" : "Reduzir"}</span>
              ...
              <div className="space-y-1">
                ...
                <div className="relative h-2 bg-muted rounded-full overflow-hidden">
                  ...
                  />
                  <div
                    className="absolute h-full bg-green-500 transition-all opacity-70"
                    style={{ width: `${allocation.newPercentage}%` }}
                  />
                </div>
              </div>
            </div>
