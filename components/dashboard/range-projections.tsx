...
export function RangeProjections({
  ...
  const hasRealData = projectedWealth > 0 || requiredWealth > 0
  ...
  const ageRange = hasRealData
    ? getAgeRange(financialIndependenceAge, confidenceLevel)
    : { min: 65, max: 65 }
  const wealthRange = hasRealData
    ? getWealthRange(projectedWealth, confidenceLevel)
    : { min: 0, max: 0 }
    ...
    <div className="grid gap-4 md:grid-cols-2">
          ...
          <div className="space-y-3">
            <div className="text-center">
              <div className="text-2xl font-bold">
                {hasRealData ? `Entre ${ageRange.min} e ${ageRange.max} anos` : "65 anos"}
              </div>
            ...
            {hasRealData && (
              <div className="flex items-center justify-center gap-2">
                <div className="h-2 bg-muted rounded-full w-32 relative">
                  <div
                    className={cn("h-2 rounded-full absolute", getRangeColor(confidenceLevel))}
                    style={{
            ...
            <div className="text-xs text-muted-foreground text-center">
              {hasRealData ? (
                ...
                "Adicione dados financeiros para ver projeções"
              )}
            </div>
          </div>
        </CardContent>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            {hasRealData ? "Valor estimado na idade de aposentadoria" : "Sem dados para projeção"}
          </CardDescription>
        </CardHeader>
        <CardContent>
          <div className="space-y-3">
            ...
            <div className="grid grid-cols-2 gap-4 text-sm">
              <div className="text-center p-2 bg-muted/50 rounded">
                ...
                <p className="font-medium">
                  {hasRealData ? formatCurrency(wealthRange.min) : "R$ 0,00"}
                </p>
              </div>
              <div className="text-center p-2 bg-muted/50 rounded">
                <p className="text-muted-foreground">Máximo</p>
                <p className="font-medium">
                  {hasRealData ? formatCurrency(wealthRange.max) : "R$ 0,00"}
                </p>
              </div>
            </div>
            ...
            <div className="text-xs text-muted-foreground text-center">
              {hasRealData
                ...
                : "Defina metas financeiras para calcular"}
            </div>
          </div>
        </CardContent>
      </Card>
