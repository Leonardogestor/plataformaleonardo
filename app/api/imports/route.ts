...
export async function GET(request: NextRequest) {
  try {
    ...
    const groupedByPeriod = documents.reduce((acc, doc) => {
      ...
      const allCompleted = acc[key].documents.every(d => d.status === "COMPLETED")
      const anyFailed = acc[key].documents.some(d => d.status === "FAILED")
    ...
    const imports = Object.values(groupedByPeriod).sort((a, b) => 
